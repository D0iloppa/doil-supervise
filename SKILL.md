...
description: >-
  ...
  위임하는 게 이로운 작업에서 트리거된다. 사소한 1회성 수정에는 쓰지 않는다. 라우팅 전에 각
  하위과업이 LLM 토큰으로 직접 할 일인지 결정적 코드(생성기+루프)로 짤 일인지 먼저
  판단하고(compute-over-inference), 애매하면 프로그램 작성 여부를 되묻는다. 세션 토큰이
...
### 2. 라우팅 (Route) — 하위 과업별 실행 수단 + 모델 + 근거
과업을 하위 과업으로 쪼갠다. 각 하위과업에 대해 **모델을 고르기 전에 먼저 "실행 수단"을
정하고**, LLM으로 남는 것만 모델을 배정한다. 모두 근거를 제시하며, 사용자가 수동 지정하지
않는다 — 판단은 감독 몫이다.

#### 2-0. 실행 수단 — LLM inference vs 결정적 코드(program) *(모델 축보다 먼저)*
이 하위과업이 **LLM이 토큰을 태워 직접 수행할 일인지, 프로그램을 작성해 결정적으로 돌릴
일인지**를 먼저 판단한다. 사용자가 "프로그램으로 하라"고 명시하지 않아도 감독이 먼저 고민한다.
표준용어로는 *compute-over-inference*(토큰 대신 결정적 연산), 데이터의 경우
*programmatic generation*(생성기 + 루프) 이다.

- **프로그램(코드-우선)이 유리한 신호**: 대량 반복(N이 큼) · 규칙으로 표현 가능 · 결정적 ·
  재현성/검증 필요 · 동일 연산의 반복. → 생성기(generator)와 루프로 처리한다.
  데이터 시딩이 전형이다. 예 — 학생 1000명 시드:
  - *하수*: 1000명을 LLM 토큰으로 한 명씩 생성 — 결과는 나오나 매우 비싸다.
  - *중수*: 시드 프로그램을 만들되 항목마다 input 을 LLM 이 생성해 호출 — 시딩 로직은
    코드지만 input 을 여전히 토큰으로 만들어 절감이 제한적이다.
  - *고수*: `param_generator()`(입력도 결정적 생성) + `seed_generator(input)` 을 만들고
    `for (i=0;i<1000;i++){ seed_generator(param_generator()); }` — **루프 자체는 토큰 0**.
    규칙화 가능한 입력(이름·범위·조합·랜덤)은 input 생성까지 코드로 내린다.
- **중간 지점(hybrid)**: 입력의 일부가 진짜 자연어 다양성을 요구하면(예: 사실적인 자기소개
  문장), LLM 으로 **소량의 템플릿/분포를 1회 생성**한 뒤 코드로 확장한다
  (*generate-once, expand-deterministically*). 매 항목을 토큰으로 만들지 않는다.
- **인라인(LLM)이 맞는 신호**: 1회성 · 소량 · 매 항목이 고유한 자연어 판단/창의성을 요구 ·
  규칙화 비용이 프로그램 작성보다 큼.
- **오버엔지니어링 경계(카파시 #2 Simplicity First)**: 즉답 가능한 소량 작업에 일반화 프로그램을
  짜지 않는다. 예 — "구구단 3단 출력"이면 그냥 3단을 출력한다. `구구단(n)` 을 짜서 `구구단(3)`
  을 호출하는 건 과설계다. 임계는 **반복 규모 × 규칙화 가능성** — 둘 다 크면 코드, 둘 다 작으면
  인라인.
- **판단이 안 서면 되묻는다(카파시 #1).** 예: "이 작업은 param_generator + seed_generator 를
  만들어 루프로 돌리는 게 토큰·재현성 면에서 효율적일 것 같습니다 — 프로그램으로 수행할
  아이디어가 있으신가요, 아니면 인라인으로 채울까요?" 처럼 도구 호출자에게 명시적으로 확인한다.
- 프로그램으로 가기로 하면, 그 **"생성기/시더 작성" 자체가 하나의 구현 하위과업**이 되어 아래
  모델 라우팅을 그대로 탄다(대개 sonnet). 그 뒤 실행(루프)은 코드로 돌리며 토큰을 태우지 않는다.

#### 2-1. 모델 배분 *(LLM 으로 남는 하위과업에 한해)*
