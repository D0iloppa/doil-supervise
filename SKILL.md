...
> 원칙: 가정하지 말 것·단순성 우선·수술적 변경·목표주도(카파시 4원칙). 레포에 자체 에이전트
> 지침(CLAUDE.md 등)이 있으면 그것을 **함께** 따른다. 이 스킬은 "어떻게 위임할지"의 절차이며,
> 개별 과업의 산출물이 아니다.

## 왜 이 스킬인가

사용자는 매번 "네가 직접 하지 말고 서브에이전트로 시켜줘 / 모델 골라서 / 이게 무슨 작업인지도
표준용어로 알려줘"를 손으로 붙여 넣는 걸 번거로워한다. 이 스킬은 그 지시를 상시화한다.
...
기본 배분(레포 지침이 다르면 그쪽 우선):
- 사소한 시각·문구 수정(CSS px·폰트 크기 조정, 문구/카피 변경, 색상 토큰 교체) → **haiku**
  — 위임 오버헤드가 과업보다 클 수 있으니, 감독이 인라인 처리를 제안해도 된다(0단계 push back).
- 검색·탐색·문서정리·기계적 수정(i18n, 패턴 미러링, 단순 제거) → **haiku / sonnet**
- 복잡 로직·설계·머니 경로(결제/보상/인증)·UI·디자인 고퀄 → **fable / opus**
- 디자인 개선은 sonnet 으로 레퍼런스 리서치 → fable 이 구현하는 2단 패턴이 유효.
...
- 각 호출에 `model`(sonnet/opus/haiku/fable) 오버라이드를 명시하고, 근거를 함께 남긴다.
- **프롬프트에 도구·컨텍스트 사용을 명시적으로 지시한다.** 서브에이전트는 부모 컨텍스트를
  물려받지 않는다 — 필요한 파일 경로, "레포에 codebase-memory MCP 있으면 그래프 조회를
  먼저 써라", "CLAUDE.md 규약을 따르라" 등을 프롬프트에 직접 써 준다.
...
이미 워커가 돌고 있는데 추가 요구·변경·중지가 들어오면, 새 오케스트레이션을 처음부터
다시 돌리지 말고 **in-flight 워커를 조정**한다. `$ARGUMENTS` 의 첫 토큰으로 분기한다.
(콜론 하위커맨드가 아니라 **인자 모드**다 — `/doil-supervise add ...` 처럼 띄어 쓴다.)
...
워커는 반드시 **명명·라우팅 근거**를 거친다(기본 절차와 동일).
...
4) 위임   → Agent(analysis) → [읽고] → Agent(implementation) [→ reviewer]
5) 종합   → 결과·검증상태 사실 보고

진행 중: add(워커 추가) / edit(방향변경·중지) / stop / status
```

## 하지 말 것
- 감독이 직접 코드를 짜는 것(위임이 과한 사소 과업이라 사용자가 인라인에 동의한 경우 제외).
- 모델을 근거 없이 고르는 것.
- 명명 단계 생략(사용자의 어휘 학습이 이 스킬의 핵심 목적 중 하나다).
- 서브에이전트에 컨텍스트·도구 지시 없이 막연한 프롬프트를 던지는 것.
